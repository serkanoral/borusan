...
train <- train %>% 
  mutate(Usage_day = as.numeric(Last_day - First_invoice))
...
train <- train %>% 
  select(-ServiceInvoiceDate, -ServiceInvoiceLineNo,-Milage_per_main, -Milage_per_main)

test <- test %>% 
  select(-ServiceInvoiceDate, -ServiceInvoiceLineNo,-Milage_per_main, -Milage_per_main,-IsMalfunction)

test$maintenance_count <- as.integer(ceiling(test$maintenance_count))
test$Last_milage <- as.integer(ceiling(test$Last_milage))
test$Usage_day <- as.integer(ceiling(test$Usage_day))
```
...
MainGroupCode
SubGroupCode
Milage - ALREADY CHANGED IN TEST
maintenance_count - ALREADY CHANGED IN TEST
Last_milage - ALREADY CHANGED IN TEST
Last_day - ALREADY CHANGED IN TEST
First_invoice - ALREADY CHANGED IN TEST
Usage_day - ALREADY CHANGED IN TEST

*Car and customer Features*
City
CustomerId
VehicleId
Make
...
```{r}
train <- train %>% 
  select(-MainGroupDescription,-SubGroupDescription, -SplitCode,-Split_Grubu,-ItemDescription,
          -ItemNo,ItemType, -MainGroupCode,-SubGroupCode)
...
rf_spec <- rand_forest(mtry = 3, trees = 1000,min_n = 13) %>% 
  set_engine("ranger") %>% 
  set_mode("classification")
